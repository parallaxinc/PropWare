...
#SRLM: Edited to use a class "UnityTests" to hold the tests. This will allow
#tested classes to keep things private and just make "UnityTests" a friend.
#Note that now the tests should be declared in *.test.h .
...
class UnityTestRunnerGenerator
  ...
  def find_tests(input_file)
    ...
    lines.each_with_index do |line, index|
      #find tests
      #SRLM: Added "static" to allow for use in a class.
      if line =~ /^((?:\s*TEST_CASE\s*\(.*?\)\s*)*)\s*static void\s+(test.*?)\s*\(\s*(.*)\s*\)/
        arguments = $1
        name = $2
        call = $3
        args = nil
  ...
  def create_runtest(output, used_mocks)
    ...
    output.puts("      UnityTests::setUp(); \\")
    output.puts("      UnityTests::TestFunc(#{va_args2}); \\")
    ...
    output.puts("  { \\")
    output.puts("    UnityTests::tearDown(); \\")
    output.puts("  } \\")
...
#  end
  ...
  ARGV[1] = ARGV[0].gsub(".test.h",".generate.cpp") if (!ARGV[1])
